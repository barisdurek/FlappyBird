...
private:
    ...
    // Bird components (more bird-like design)
    sf::RectangleShape birdBody;  // Slightly rectangular body
    sf::ConvexShape birdWing;     // Curved, feathered wing
    sf::ConvexShape birdBeak;     // Pointed beak
    sf::CircleShape birdEye;      // Eye with pupil
    sf::CircleShape birdPupil;    // White pupil for detail
    int birdFrame;
...
public:
    ...
    void initGame() {
        ...
        // Bird setup (more bird-like)
        birdBody.setSize(sf::Vector2f(30, 20));  // Rectangular body (wider than tall)
        birdBody.setFillColor(sf::Color(255, 215, 0));  // Yellow body
        birdBody.setOutlineThickness(1);
        birdBody.setOutlineColor(sf::Color::Black);
        birdBody.setOrigin(15, 10);  // Center for rotation

        birdWing.setPointCount(5);  // Curved wing shape
        birdWing.setPoint(0, sf::Vector2f(0, 0));
        birdWing.setPoint(1, sf::Vector2f(-15, -5));  // Upward curve
        birdWing.setPoint(2, sf::Vector2f(-20, 10));  // Wing tip
        birdWing.setPoint(3, sf::Vector2f(-10, 15));  // Downward curve
        birdWing.setPoint(4, sf::Vector2f(0, 10));    // Base
        birdWing.setFillColor(sf::Color(255, 165, 0));  // Orange wing

        birdBeak.setPointCount(3);  // Pointed beak
        birdBeak.setPoint(0, sf::Vector2f(0, 0));
        birdBeak.setPoint(1, sf::Vector2f(12, -2));  // Longer and pointed
        birdBeak.setPoint(2, sf::Vector2f(12, 2));
        birdBeak.setFillColor(sf::Color(255, 69, 0));  // Reddish-orange beak

        birdEye.setRadius(3);  // Slightly larger eye
        birdEye.setFillColor(sf::Color::Black);
        birdPupil.setRadius(1);  // Small white pupil
    ...
    void checkCollisions() {
        ...
        if (birdY + 10 >= groundHeight) {  // Half of body height (20/2)
        ...
        for (int i = 0; i < PIPE_COUNT; i++) {
            if (BIRD_X + 15 >= pipes[i].x && BIRD_X - 15 <= pipes[i].x + PIPE_WIDTH &&  // Half of body width (30/2)
    ...
    void draw(sf::RenderWindow& window) {
        ...
        for (int i = 0; i < PIPE_COUNT; i++) {
            ...
            window.draw(bottomPipe);  // Corrected to bottomPipe
        ...
        birdWing.setPosition(BIRD_X + 10, birdY + (birdFrame ? -5 : 5));  // Adjusted for wing flapping
        birdWing.setRotation(birdAngle);
        birdBeak.setPosition(BIRD_X + 15, birdY);  // Beak at the front
        birdBeak.setRotation(birdAngle);
        birdEye.setPosition(BIRD_X + 10, birdY - 5);  // Eye near the front
        birdPupil.setPosition(BIRD_X + 11, birdY - 5);  // Pupil slightly offset
